...
router::Router JsonReader::FillRouterData(const json::Node& data) const{
    return router::Router{ data.AsDict().at("bus_wait_time").AsInt(), data.AsDict().at("bus_velocity").AsDouble()};
}
...
const json::Node JsonReader::PrintRouter(const json::Dict& request, RequestHandler& dos) const{
    json::Node result;
    const int id = request.at("id").AsInt();
    const auto& routing = dos.GetRouter().BuildRoute(request.at("from").AsString(),request.at("to").AsString());
    ...
    if(!routing.has_value()){
        ...
        for(auto& edge_id : routing.value().edges){
            const graph::Edge<double> edge = dos.GetRouter().GetGraph().GetEdge(edge_id);
